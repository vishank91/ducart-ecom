...
async function createRecord(req, res) {
    try {
        ...
        mailer.sendMail({
            ...
            subject: "Your Order Has Been Placed : Team Dcart",
            text: `
    ...
    try {
        ...
        if (data) {
            ...
            mailer.sendMail({
                from: process.env.MAIL_SENDER,
                to: finalData.user.email,
                subject: "Your Order Has Been Updated : Team Dcart",
                text: `
                                Hello ${finalData.user.name},
                                Your Order Has Been Updated
                                Please Track Your Order in Profile Section
                                Team : Dcart
                            `
            }, (error) => {
                if (error)
        ...
        error.keyValue ?
            res.send({ result: "Fail", reason: "Checkout Name is Already Exist" }) :
            res.status(500).send({ result: "Fail", reason: "Internal Server Error" })
    }
}
